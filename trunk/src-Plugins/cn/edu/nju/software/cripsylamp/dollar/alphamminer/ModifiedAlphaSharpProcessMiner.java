...
//
//		int nInvNum = 0;
//		Hashtable<String,ArrayList<DoubleSet>> htSucc = new Hashtable<String,ArrayList<DoubleSet>>();
//        Hashtable<String,ArrayList<DoubleSet>> htPred = new Hashtable<String,ArrayList<DoubleSet>>();
...
//            {
//                ArrayList<DoubleSet> alSuccPred = genInputPlaces(ps.getSucc());
...
//            {
//                ArrayList<DoubleSet> sarPredSucc = genOutputPlaces(ps.getPred());
...
//        {
//        	PredSucc ps = (PredSucc) alFalseDep.get(i);
//            ArrayList<DoubleSet> sarPred = (ArrayList<DoubleSet>) htPred.get("" + ps.getPred());
//            ArrayList<DoubleSet> sarSucc = (ArrayList<DoubleSet>) htSucc.get("" + ps.getSucc());
//            for (int j=0; j<sarPred.size(); j++)
//            {
//            	DoubleSet dsPred = sarPred.get(j);
...
//            			 for (String tname	:	invTaskAXYB.keySet())
//            			 {
//            				 AXYB axyb1 = invTaskAXYB.get(tname);
//            				 if (axyb.equals(axyb1))
//            				 {
//            					 taskName = tname;
//            				 }
//            			 }
//            			 if (taskName != null)
//            			 {
//            				 InvisibleTask it1 = null;
//            				 for (int m=0; m<alInvTask.size(); m++)
//            				 {
//            					 if (((InvisibleTask)alInvTask.get(m)).getName().equals(taskName))
//            					 {
//            						 it1 = (InvisibleTask)alInvTask.get(m);
//            					 }
//            				 }
//            				 if (it1!= null)
//            				 {
...
//
//		/*
//		ArrayList<RelationInSkipRedo> toAddedRelation = new ArrayList<RelationInSkipRedo>();
...
//                    		if (!sarPredPred.contains(a) ||
//                    			!sarPredSucc.contains(x) ||
//                    			!sarSuccPred.contains(y) ||
//                    			!sarSuccSucc.contains(b))
//                    			continue;
//                    	}
//                        String t = "__skip_redo_" + (nInvNum + 1) + "__";
//
...
//                        {
//
//                        	if (notNormalInvTaskSet.get(ps) != null)
//                        	{
//                        		int num = nInvNum+nme;
//	                            ArrayList<Integer> ac = notNormalInvTaskSet.get(ps).afterCausal;
//	                            ArrayList<Integer> bc = notNormalInvTaskSet.get(ps).beforeCausal;
//	                            ArrayList<Integer> sr = notNormalInvTaskSet.get(ps).sharpRelation;
//	                            for (Integer	a	:	ac)
//	                            {
//	                            	RelationInSkipRedo relation = new RelationInSkipRedo();
//	                            	relation.from = num;
//	                            	relation.to = a;
//	                            	relation.Relation = relation.Causal;
//	                            	toAddedRelation.add(relation);
//	                            }
//	                            for (Integer	b	:	bc)
//	                            {
//	                            	RelationInSkipRedo relation = new RelationInSkipRedo();
...
//	                            	relation.from = a;
//	                            	relation.to = num;
//	                            	relation.Relation = relation.Sharp;
//	                            	toAddedRelation.add(relation);
//
//	                            	relation = new RelationInSkipRedo();
//	                            	relation.from = num;
//	                            	relation.to = a;
//	                            	relation.Relation = relation.Sharp;
//	                            	toAddedRelation.add(relation);
...
//
//      //toAdd relation
//     	for (RelationInSkipRedo relation	:	toAddedRelation)
//     	{
//     		if (relation.Relation == RelationInSkipRedo.Causal)
...
//                if (isParaPred || isParaSucc)
//                {
//                	//可能invisible task和某些捣乱的节点已经是因果或sharp关系了，就不可以是并行关系了
//                	boolean tag = false;
//                	for (int j=0; j<toAddedRelation.size(); j++)
//                	{
//                		int x = toAddedRelation.get(j).from;
//                		int y = toAddedRelation.get(j).to;
//                		if (toAddedRelation.get(j).Relation != RelationInSkipRedo.Causal &&
//                				toAddedRelation.get(j).Relation != RelationInSkipRedo.Sharp)
//                			continue;
//                		if ((x == iti.id && k ==y) || (y == iti.id && k ==x))
//                			tag = true;
//                	}
//                	if (tag)
//                		continue;
...
//    {
//    	HashSet<InvisibleTask> toDelete = new HashSet<InvisibleTask>();
...
//    }
//
//    private boolean checkISYCausalXAsequenceB(int a, int x, int y, int b, HashSet<Integer> misc,
...
//				boolean checkPar;
//				HashSet<HashSet<Integer>> content = intervalTasksSet.getContent(y,x);
...
//
//    /*
//    private ArrayList getFalseDep(ArrayList<PredSucc> alFalseDep, LogReader log)
//    {
...
//    									PredSucc ps = new PredSucc(a, b,i,y);
...
//    					{
//    						 PredSucc ps = new PredSucc(a, b,x,y);
//                             if (!alFalseDep.contains(ps))
//                             {
//                                 alFalseDep.add(ps);
...
//                    if (tj == psk.getPred())
//                    {
//                    	for (PredSucc ps	:	alFalseDep)
//                    	{
//                    		if (ps.getPred() == psi.getPred() && ps.getSucc() == psk.getSucc())
//                    			if (!alRedunDep.contains(ps))
//                                {
//                                    alRedunDep.add(ps);
//                                }
//                    	}
...
//    {
//    	HashSet<PredSucc> needToDelete = new HashSet<PredSucc>();
...
//    		}
//    	}
//		HashSet<Integer> sharps = AlphaMMinerDataUtil.except(allTasks, availableTask);
...
//	}
//
//	private boolean checkExistPath(int a, int b, HashSet<Integer> misc, LogReader log, ArrayList<ProcessInstance> selectedProcess, ArrayList<Integer> selectedInterfereTask) {
...
//	}
//
//	private boolean checkParallel(HashSet<Integer> misc, HashSet<Integer> betweenXY,
//			int x, int y, HashSet<HashSet<Integer>> content) {
...
//        		numFirstInv += ds.getA().size() * ds.getB().size();
//        	}
//        	for (int i=0; i<alLastGroup.size(); i++)
//        	{
//        		DoubleSet ds = (DoubleSet) alLastGroup.get(i);
//        		numLasttInv += ds.getA().size() * ds.getB().size();
//        	}
//            int nmeNew = nme + numFirstInv + numLasttInv;
//            DoubleMatrix1D dmOneLoop = DoubleFactory1D.sparse.make(nmeNew, 0);
//            for (int i = 0; i < nme; i++)
//                dmOneLoop.set(i, relations.getOneLengthLoopsInfo().get(i));
//            DoubleMatrix2D dmCausal = DoubleFactory2D.sparse.make(nmeNew, nmeNew, 0);
...
//            {
//            	DoubleSet ds = (DoubleSet) alFirstGroup.get(i);
//            	IntArrayList sarPre = ds.getA();
//            	IntArrayList sarSucc = ds.getB();
//                for (int j = 0; j < sarSucc.size(); j++)
//                {
//                    dmStart.set(sarSucc.get(j), 0);
//                }
//            	for (int m = 0; m < sarPre.size(); m++)
//            	{
//            		for (int n=0; n < sarSucc.size(); n++)
//            		{
//            			int id = nme + startCount;
//            			startCount ++;
//                        String t = "__begin_" + startCount + "__";
//
//                        //add to log events
//                        LogEvent le = new LogEvent(t, "auto");
//                        leEvents.add(le);
//
//                        //add to relation mxatrix
//                        for (int j = 0; j < sarSucc.size(); j++)
//                        {
//                            dmCausal.set(id, sarSucc.get(j), 1);
//                        }
...
//                        int y = sarPre.get(m);
//                        int b = sarSucc.get(n);
//                        String task_a = "a_begin";
//                        String task_x = "x_begin";
//                        String task_y = relations.getLogEvents().get(y).getModelElementName();
//                        String task_b = relations.getLogEvents().get(b).getModelElementName();
//                        AXYB axyb = new AXYB(task_a, task_x, task_y, task_b);
//                        invTaskAXYB.put(t, axyb);
...
//                	for (int n=0; n<sarSucc.size(); n++)
//                	{
//                		int id = nme + startCount + endCount;
//                		endCount ++;
//                        String t = "__end_" + (endCount + 1) + "__";
//
//                        //add to log events
//                        LogEvent le = new LogEvent(t, "auto");
//                        leEvents.add(le);
...
//                        for (int j = 0; j < sarPre.size(); j++)
//                        {
//                            dmCausal.set(sarPre.get(j), id, 1);
//                        }
//                        dmEnd.set(id, 1);
//                        //Add the Inv task
...
//
//			//1.put all names into the secont part.
//			int size = ates.size();
//			for (int i=0; i<size; i++)
//			{
//				try
...
//			for (int i=0; i<size; i++)
//			{
//				String nowTask = secondPart.get(0);
//				secondPart.remove(0);
//				for (String preTask :	firstPart)
//					for (String postTask	:	secondPart)
//						intervalTasksSet.addTask(preTask, postTask, nowTask, relations.getLogEvents());
//				firstPart.add(nowTask);
//			}
//			*/
//
...
//	}
//
//	public AlphaPPData mineAlphaSharpInfo(LogReader log, LogRelations theRelations, Progress progress)
//	{
