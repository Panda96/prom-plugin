...
//        clearAll();
//        Progress progress = new Progress("Mining " + log.getFile().getShortName() +
//                " using " + getName());
...
//        Petrinet petrinet = PetrinetFactory.newPetrinet("new petrinet");
//        //all transitions
...
//
//            petrinet.addTransition(new LogEvent(strT, strType));
...
//            petrinet.addArc(psrc, (Transition) htTrans.get(alT_I.get(i)));
//        }
//        //sink place
//        Place psink = petrinet.addPlace("psink");
//        //last transitions to sink place
//        for (int i = 0; i < alT_O.size(); i++) {
//            petrinet.addArc((Transition) htTrans.get(alT_O.get(i)), psink);
//        }
...
//                petrinet.addArc((Transition) htTrans.get(ds.alA.get(j)), p);
//            }
//            //outgoing arcs
//            for (int j = 0; j < ds.alB.size(); j++) {
//                petrinet.addArc(p, (Transition) htTrans.get(ds.alB.get(j)));
//            }
//        }
//
//
...
//     * @param availableY
//     */
//    private void getAvailableXY(Petrinet petrinet, Transition t, HashSet<String> availableX, HashSet<String> availableY, HashSet<String> availableA, HashSet<String> availableB) {
//        HashSet<Place> prePlaces = PreSuccHelper.getPrePlaces(petrinet, t);
//        HashSet<Place> succPlaces = PreSuccHelper.getSuccPlaces(petrinet, t);
//
//        for (Place prePlace : prePlaces) {
//            HashSet<Transition> xs = PreSuccHelper.getPreTransitions(petrinet, prePlace);
//            HashSet<Transition> as = PreSuccHelper.getSuccTransitions(petrinet, prePlace);
//            for (Transition x : xs) {
//                if (x.equals(t))
//                    continue;
//                availableX.add(x.getLabel());
//            }
//            for (Transition a : as) {
//                if (a.equals(t))
//                    continue;
//                availableA.add(a.getLabel());
//            }
//        }
//
//        for (Place succPlace : succPlaces) {
//            HashSet<Transition> ys = PreSuccHelper.getPreTransitions(petrinet, succPlace);
//            HashSet<Transition> bs = PreSuccHelper.getSuccTransitions(petrinet, succPlace);
//            for (Transition y : ys) {
//                if (y.equals(t))
//                    continue;
//                availableY.add(y.getLabel());
//            }
//            for (Transition b : bs) {
//                if (b.equals(t))
//                    continue;
//                availableB.add(b.getLabel());
//            }
//        }
//    }
//
...
//            if (transition.getLabel() != null || !transition.isInvisible())
//                continue;
...
//            getAvailableXY(petrinet, transition, availableX, availableY, availableA, availableB);
...
//                }
//            }
//            invTaskWithAXYB.put(transition.getLabel(), _axybs);
//        }
//
//    }
//
...
//				{
//					ArrayList<String> _ASet;
//					ArrayList<String> _BSet;
//					AXYB a_AXYB = invTaskAXYB.get(t1.getIdentifier());
//					AXYB b_AXYB = invTaskAXYB.get(t2.getIdentifier());
//					_ASet = AlphaMMinerDataUtil.union(a_AXYB.x,a_AXYB.y);
//					_BSet = AlphaMMinerDataUtil.union(b_AXYB.x,b_AXYB.y);
...
//    */
//
//    private boolean isCheckInterSequence(Petrinet petrinet, Transition t1, Transition t2) {
//        HashSet<Place> preT1 = PreSuccHelper.getPrePlaces(petrinet, t1);
//        HashSet<Place> succT1 = PreSuccHelper.getSuccPlaces(petrinet, t1);
//        HashSet<Place> preT2 = PreSuccHelper.getPrePlaces(petrinet, t2);
//        HashSet<Place> succT2 = PreSuccHelper.getSuccPlaces(petrinet, t2);
//
...
//            HashSet<Transition> ts = PreSuccHelper.getPreTransitions(petrinet, place);
//            preT1ts.addAll(ts);
//        }
//
//        for (Place place : succT1) {
//            HashSet<Transition> ts = PreSuccHelper.getSuccTransitions(petrinet, place);
//            succT1ts.addAll(ts);
//        }
//
...
//        HashSet<Transition> joinPre = AlphaMMinerDataUtil.join(preT1ts, preT2ts);
//        preT1ts = AlphaMMinerDataUtil.except(preT1ts, joinPre);
//        preT2ts = AlphaMMinerDataUtil.except(preT2ts, joinPre);
//
//        HashSet<Transition> joinSucc = AlphaMMinerDataUtil.join(succT1ts, succT2ts);
...
//            if (ta.getLabel() == null || ta.isInvisible())
//                continue;
//            for (Transition tb : succT2ts) {
//                if (tb.getLabel() == null || ta.isInvisible())
//                    continue;
...
//        //for each set, combine it.
//        for (int i = 0; i < transitionSets.size(); i++) {
//            HashSet<Place> prePlaces = new HashSet<Place>();
//            HashSet<Place> postPlaces = new HashSet<Place>();
//            ArrayList<Transition> transitionSet = transitionSets.get(i);
//            Transition first = transitionSet.get(0);
//            for (int j = 1; j < transitionSet.size(); j++) {
//                Transition t = transitionSet.get(j);
//                HashSet<Place> pre = PreSuccHelper.getPrePlaces(petrinet, t);
//                HashSet<Place> post = PreSuccHelper.getSuccPlaces(petrinet, t);
//                prePlaces.addAll(pre);
//                postPlaces.addAll(post);
//                petrinet.removeNode(t);
//            }
//            for (Place pre : prePlaces) {
//                if (petrinet.getArc((Place) pre, (Transition) first) == null)
//                    petrinet.addArc((Place) pre, (Transition) first);
//            }
//            for (Place post : postPlaces) {
//                if (petrinet.getArc((Transition) first, (Place) post) == null)
//                    petrinet.addArc((Transition) first, (Place) post);
//            }
//        }
//    }
//
...
//                if (invTask.taskName.equals(transition.getLabel())) {
//                    isInv = true;
//                    break;
//                }
//            if (!isInv)
...
//        HashMap<HashSet<Transition>, HashMap<HashSet<Transition>, ArrayList<Place>>> places =
//                new HashMap<HashSet<Transition>, HashMap<HashSet<Transition>, ArrayList<Place>>>();
//        for (Place place : petriNet.getPlaces()) {
//            HashSet<Transition> preTransitions = PreSuccHelper.getPreTransitions(petriNet, place);
//            HashSet<Transition> postTransitions = PreSuccHelper.getSuccTransitions(petriNet, place);
//            HashMap<HashSet<Transition>, ArrayList<Place>> singleLine = places.get(preTransitions);
//            if (singleLine == null) {
...
//            line.add(place);
//        }
//        for (HashMap<HashSet<Transition>, ArrayList<Place>> singleLine : places.values()) {
//            for (ArrayList<Place> line : singleLine.values()) {
//                if (line.size() > 1) {
//                    for (int i = 1; i < line.size(); i++)
//                        petriNet.removeNode(line.get(i));
//                }
//            }
//        }
//    }
//    /*
//    private void combineInvisibleTaskSup(PetriNet petriNet)	//for the parallel hehe...
//    {
//    	ArrayList<Transition> rawInvisibleTask = new ArrayList<Transition>();
//    	ArrayList<Transition> combinedInvisibleTask = new ArrayList<Transition>();
//    	for (Transition transition	:	petriNet.getTransitions())
//    	{
//    		if (transition.getLogEvent() != null)
//    			continue;
//    		rawInvisibleTask.add(transition);
//    	}
//
//    	for (int i=0; i<rawInvisibleTask.size(); i++)
//    	{
//    		Transition it = rawInvisibleTask.get(i);
//    		HashSet<Place> dsPred = it.getPredecessors();
//    		HashSet<Place> dsSucc = it.getSuccessors();
//    		int nCombined = 0;
//    		for (int j=0; j<combinedInvisibleTask.size(); j++)
//    		{
//    			Transition iti = combinedInvisibleTask.get(j);
//    			boolean isInPred = iti.getPredecessors().containsAll(dsPred);
//    			boolean isInSucc = iti.getSuccessors().containsAll(dsSucc);
...
//    			else if (isInPred)
//    			{
//    				boolean isCombined = isCombinable(iti.getPredecessors(), it.getSuccessors(), iti.getSuccessors(), it.getSuccessors());
//    				if (isCombined)
//    				{
//    					//将it的内容添加到
//    					//1.将it的前驱place添加到iti上
//    					for (Place p	:	(HashSet<Place>)it.getPredecessors())
//    					{
//    						if (petriNet.getEdgesBetween((Place) p, (Transition) iti).size() == 0)
//    						{
//    							petriNet.addEdge((Place) p, (Transition) iti);
//    						}
//    					}
//    					//2.将it的后继place添加到it上
//    					for (Place p	:	(HashSet<Place>) it.getSuccessors())
//    					{
//    						if (petriNet.getEdgesBetween((Transition) iti, (Place) p).size() == 0)
//    						{
//    							petriNet.addEdge((Transition) iti, (Place) p);
//    						}
//    					}
//    					//3.axyb
//    					AXYB axybITI = invTaskAXYB.get(iti.getIdentifier());
//    					AXYB axybIT = invTaskAXYB.get(it.getIdentifier());
//    					axybITI.combine(axybIT);
//    					nCombined ++;
//    				}
//    			}
//    			else if (isInSucc)
//    			{
//    				boolean isCombined = isCombinable(it.getPredecessors(), iti.getSuccessors(), it.getPredecessors(), iti.getPredecessors());
//    				if (isCombined)
//    				{
//    					//将it的内容添加到
//    					//1.将it的前驱place添加到iti上
...
//    			{
//    				boolean isCombined = isCombinable(iti.getPredecessors(), it.getSuccessors(), iti.getSuccessors(), it.getSuccessors());
//    				isCombined = isCombined && isCombinable(it.getPredecessors(), iti.getSuccessors(), it.getPredecessors(), iti.getPredecessors());
//    				if (isCombined)
//    				{
//    					//将it的内容添加到
//    					//1.将it的前驱place添加到iti上
...
//    		{
//				//4.删除it
//				petriNet.delTransition(it);
//				continue;
//    		}
//    		for (int j=i+1; j<rawInvisibleTask.size(); j++)
//    		{
//    			Transition iti = rawInvisibleTask.get(j);
//    			boolean isInPred = iti.getPredecessors().containsAll(dsPred);
//    			boolean isInSucc = iti.getSuccessors().containsAll(dsSucc);
//    			if (isInPred && isInSucc)
//    			{
...
//    		else if (nCombined == 0)
//    		{
//    			combinedInvisibleTask.add(it);
//    		}
//    	}
//    }
//    */
//
//    private boolean isCombinable(Petrinet petrinet, HashSet<Place> sarIPredH, HashSet<Place> sarSuccH,
//                                 HashSet<Place> sarISuccH, HashSet<Place> sarPredH) {
...
//            HashSet<Transition> sarIPredPredH = PreSuccHelper.getPreTransitions(petrinet,sarIPred[j]);
//            Transition[] sarIPredPred = sarIPredPredH.toArray(new Transition[0]);
//            for (int k = 0; k < sarIPredPred.length; k++) {
//                if (sarIPredPred[k].getLabel() == null)
//                    continue;
//                for (int n = 0; n < sarSucc.length; n++) {
//                    HashSet<Transition> sarSuccSuccH = PreSuccHelper.getSuccTransitions(petrinet,sarSucc[n]);
//                    Transition[] sarSuccSucc = sarSuccSuccH.toArray(new Transition[0]);
//                    for (int m = 0; m < sarSuccSucc.length; m++) {
//                        if (sarSuccSucc[m].getLogEvent() == null)
//                            continue;
//                        String preName = sarIPredPred[k].getLabel() + '\0' + sarIPredPred[k].getLogEvent().getEventType();
//                        String succName = sarSuccSucc[m].getLabel() + '\0' + sarSuccSucc[m].getLogEvent().getEventType();
//                        boolean isContained = false;
//                        for (InvTask invTask : invTasks) {
//                            if (invTask.pre.contains(preName) && invTask.suc.contains(succName)) {
//                                isContained = true;
//                                break;
//                            }
//                        }
...
////                if (existPara(sarIPred[j].getSuccessors(), sarSucc[k].getPredecessors()))
//                 if(existPara(PreSuccHelper.getSuccTransitions(petrinet,sarIPred[j]),PreSuccHelper.getPreTransitions(petrinet,sarSucc[k])));
//                    return false;
//        //I's Succ vs Succ
//        for (int j = 0; j < sarISucc.length; j++)
//            for (int k = 0; k < sarPred.length; k++)
//                if (!(existPara(sarISucc[j].getPredecessors(), sarPred[k].getPredecessors()) ||
//                        existPara(sarISucc[j].getSuccessors(), sarPred[k].getSuccessors())))
//                    return false;
//        return true;
//
//    }
...
//                    continue;
//                String tName1 = ti.getIdentifier() + '\0' + ti.getLogEvent().getEventType();
//                String tName2 = tj.getIdentifier() + '\0' + tj.getLogEvent().getEventType();
...
//    {
//    	HashMap<InvisibleAdjacent,ArrayList<Transition>> prePostInv =
//				new HashMap<InvisibleAdjacent,ArrayList<Transition>>();
//		for (Transition transition	:	petriNet.getTransitions())
//		{
//			if (transition.getLogEvent() != null)
//				continue;
//			HashSet<ModelGraphVertex> prePlaces = PreSuccHelper.getPrePlaces(petrinet,transition)();
//			HashSet<ModelGraphVertex> postPlaces = transition.getSuccessors();
//			InvisibleAdjacent ia = InvisibleAdjacent.getInvisibleTask(petriNet,prePlaces,postPlaces);
//			ArrayList<Transition> trans = prePostInv.get(ia);
//			if (trans == null)
//			{
//				trans = new ArrayList<Transition>();
//				prePostInv.put(ia, trans);
//			}
//			trans.add(transition);
//		}
//
//
//		for (ArrayList<Transition> trans	:	prePostInv.values())
//		{
//			if (trans.size()>1)
//			{
//				AXYB head = invTaskAXYB.get(trans.get(0).getIdentifier());
//
//				HashSet<Place> prePlaceStd = trans.get(0).getPredecessors();	//只需要这些place就够了，剩下的place都是多余的！
//				HashSet<Place> succPlaceStd = trans.get(0).getSuccessors();
//
//				for (int i=1; i<trans.size(); i++)
//				{
//					Transition t = trans.get(i);
//
//					HashSet<Place> prePlace = t.getPredecessors();	//只需要这些place就够了，剩下的place都是多余的！
//					HashSet<Place> succPlace = t.getSuccessors();
//					for (Place p	:	prePlace)
//						if (!prePlaceStd.contains(p))
//							petriNet.delPlace(p);
//					for (Place p	:	succPlace)
//						if (!succPlaceStd.contains(p))
//							petriNet.delPlace(p);
//					AXYB axyb = invTaskAXYB.get(t.getIdentifier());
//					head.combine(axyb);
//					petriNet.delTransition(t);
//				}
//			}
...
//
//    	//TODO
//		HashMap<HashSet<ModelGraphVertex>,HashMap<HashSet<ModelGraphVertex>,ArrayList<Transition>>> prePostInv =
//				new HashMap<HashSet<ModelGraphVertex>,HashMap<HashSet<ModelGraphVertex>,ArrayList<Transition>>>();
//		for (Transition transition	:	petrinet.getTransitions())
//		{
//			if (transition.getLogEvent() != null)
//				continue;
//			HashSet<ModelGraphVertex> prePlaces = PreSuccHelper.getPrePlaces(petrinet,transition)();
//			HashSet<ModelGraphVertex> postPlaces = transition.getSuccessors();
//			HashMap<HashSet<ModelGraphVertex>,ArrayList<Transition>> postInv = prePostInv.get(prePlaces);
//			if (postInv == null)
//			{
//				postInv = new HashMap<HashSet<ModelGraphVertex>,ArrayList<Transition>>();
//				prePostInv.put(prePlaces,postInv);
//			}
//
//			ArrayList<Transition> trans = postInv.get(postPlaces);
//			if (trans == null)
//			{
//				trans = new ArrayList<Transition>();
//				postInv.put(postPlaces, trans);
//			}
//			trans.add(transition);
//		}
//
//		for (HashMap<HashSet<ModelGraphVertex>,ArrayList<Transition>> postInv : prePostInv.values())
//		{
//			for (ArrayList<Transition> trans	:	postInv.values())
//			{
//				if (trans.size()>1)
//				{
//					AXYB head = invTaskAXYB.get(trans.get(0).getIdentifier());
...
//			//1.去除 logevent
//			transition.setLogEvent(null);
//			AXYB axyb = invTaskAXYB.get(transition.getIdentifier());
//			if (axyb == null)
//				continue;
//
//			//a->x
//			HashSet<ModelGraphVertex> prePlaces = PreSuccHelper.getPrePlaces(petrinet,transition)();
//			for (ModelGraphVertex prePlace	:	prePlaces)
//			{
//				HashSet<ModelGraphVertex> prePlacePostTasks = prePlace.getSuccessors();
//				for (ModelGraphVertex task	:	prePlacePostTasks)
//				{
//					if (!task.getIdentifier().equals(axyb.x.get(0)))
//						continue;
//
//					if (task.getPredecessors().size() > 1)
//					{
//						for (ModelGraphVertex otherPlace	:	(HashSet<ModelGraphVertex>) task.getPredecessors())
//						{
//							if (otherPlace.equals(prePlace))
//								continue;
//							if (!checkPlaceInIDW3(otherPlace))		//since only the new place in IDW3 should be adjusted.
//								continue;
//							if (petrinet.getEdgesBetween((Place) otherPlace, (Transition) transition).size() ==0)
//								petrinet.addEdge((Place) otherPlace, (Transition) transition);
//						}
//					}
//				}
//			}
...
//			//y->b
//			HashSet<ModelGraphVertex> postPlaces = transition.getSuccessors();
//			for (ModelGraphVertex postPlace	:	postPlaces)
//			{
//				HashSet<ModelGraphVertex> postPlacePreTasks = postPlace.getPredecessors();
//				for (ModelGraphVertex task	:	postPlacePreTasks)
//				{
//					if (!task.getIdentifier().equals(axyb.y.get(0)))
//						continue;
//					if (task.getSuccessors().size() > 1)
//					{
//						for (ModelGraphVertex otherPlace	:	(HashSet<ModelGraphVertex>) task.getSuccessors())
//						{
//							if (otherPlace.equals(postPlace))
//								continue;
//							if (!checkPlaceInIDW3(otherPlace))		//since only the new place in IDW3 should be adjusted.
//								continue;
//							if (petrinet.getEdgesBetween((Transition) transition,(Place) otherPlace).size() ==0)
//								petrinet.addEdge((Transition) transition,(Place) otherPlace);
//						}
//					}
//				}
//			}
...
//*/
//	/*
//	 * 重构 adjustNonfreeChoice (对应的是split)
//	 * 对于一个不可见任务A,如何重构:
//	 * 1.判断A中有多少组axyb需要添加新的place,(无论是x或者y都算数)，记录个数为num
//	 * 		1.1 如果num=0,那么不许要split
//	 * 		1.2 如果num=1,那么直接添加需要的place即可,
//	 * 		1.3如果num>1,split (num-1)次，每一次分别添加所需要的place
//	 */
//
//    private void adjustNonfreeChoiceInvisibleTask(Petrinet petriNet) {
//        int count = 0;
...
//            ArrayList<AXYB> tAXYBs = invTaskWithAXYB.get(transition.getLabel());
...
//                    if (axyb.invTasktype == AXYB.SKIP) {
//                        HashSet<Place> prePlaces = PreSuccHelper.getPrePlaces(petrinet,transition)();
...
//                        HashSet<Place> succPlaces = transition.getSuccessors();
//                        for (Place p : prePlaces) {
//                            HashSet<Transition> prePlacePreT = p.getPredecessors();
//                            for (Transition t : prePlacePreT) {
//                                if (t.getLabel().equals(axyb.a))
//                                    prePlace = p;
//                            }
//                        }
//                        for (Place p : succPlaces) {
//                            HashSet<Transition> succPlacesuccT = p.getSuccessors();
//                            for (Transition t : succPlacesuccT) {
//                                if (t.getLabel().equals(axyb.b))
//                                    succPlace = p;
//                            }
//                        }
...
//                            }
//                        }
//                        prePlace = (Place) PreSuccHelper.getPrePlaces(petriNet,transition).iterator().next();
...
////                        t.("ADDTOSKIP" + (count++));
//                        t.setLogEvent(null);
//                        t=petriNet.addTransition("ADDTOSKIP" + (count++));
...
//                return;
//            for (Place p : (HashSet<Place>) xPreSuccHelper.getPrePlaces(petrinet,transition)()) {
//                if (!PreSuccHelper.getPrePlaces(petrinet,transition)().contains(p))
...
//                    petriNet.addEdge((Place) p, (Transition) transition);
//            }
//            for (Place p : (HashSet<Place>) bPreSuccHelper.getPrePlaces(petrinet,transition)()) {
//                if (!transition.getSuccessors().contains(p))
//                    petriNet.addEdge((Transition) transition, (Place) p);
//            }
//        } else if (axyb.invTasktype == AXYB.START) {
...
//        for (String pre : pres) {
//            ArrayList<String> succs = (ArrayList<String>) alID_W3.htPred.get(pre);
//            String ppre = pre.substring(0, pre.indexOf("\0"));
//            for (String succ : succs) {
//                String ssucc = succ.substring(0, succ.indexOf("\0"));
//                HashSet<Transition> preTransitions = otherPlace.getPredecessors();
//                HashSet<Transition> succTransitions = otherPlace.getSuccessors();
//                boolean a = false;
//                boolean b = false;
...
//            }
//        }
//        HashMap<String, HashMap<String, ArrayList<InvTask>>> invisibleTaskMap = new HashMap<String, HashMap<String, ArrayList<InvTask>>>();    //The beginInvisibleTask, theEndInvisibleTask ==>InvisibleTask;
...
//    }
//
//    private void beginAdjustInvisibleTask(HashMap<String, HashMap<String, ArrayList<InvTask>>> invisibleTaskMap) {
//
//        for (InvTask invTask : invTasks) {
...
//                    HashMap<String, ArrayList<InvTask>> singleLine = invisibleTaskMap.get(preTask);
//                    if (singleLine == null) {
...
//    }
//
//    private ArrayList<AuditTrailEntryList> adjustInvisibleTask(AuditTrailEntryList atel, HashMap<String, HashMap<String, ArrayList<InvTask>>> invisibleTaskMap) {
...
//        HashMap<Integer, ArrayList<InvTask>> posInvisibleTask = new HashMap<Integer, ArrayList<InvTask>>();    //each pos correspond with a set of invisible tasks. that the set of the invisible task can be matched with the trace; between [pos, pos +1] should be the invisible tasks.
//        //1.判断所有可能添加invisibleTask的位置,存到posInvisibleTask里面
//        try {
//            //start
//            String begin = atel.get(0).getName();
//            HashMap<String, ArrayList<InvTask>> singleLine = invisibleTaskMap.get(InvTask.TRACE_BEGIN_TAG);
//            if (singleLine != null) {
...
//                              ArrayList<AuditTrailEntryList> result, AuditTrailEntryList nowList, AuditTrailEntryList basicList, HashMap<Integer, ArrayList<InvTask>> posInvisibleTask) throws IndexOutOfBoundsException, IOException {
//
...
//		// TODO Auto-generated method stub
//    	AuditTrailEntryList result = null;
//    	try
//    	{
//    		result = atel.cloneInstance();
//    		int pos = 0;
//    		while (pos <result.size())
//    		{
//    			checkAte(result,pos);		//check wether an inv-task should be insert.
//    			pos ++;
//    		}
//
//		}
...
//	}
//
//	private void checkAte(AuditTrailEntryList result, int pos) throws IndexOutOfBoundsException, IOException
//	{
//		for (InvTask invTask	:	invTasks)
//		{
//
...
//			String postLogName;
//
//			AuditTrailEntry postAte = result.get(pos);
//			postLogName = postAte.getElement() + "\0" + postAte.getType();
//
//			if (pos == 0)		//第一只
//			{
//				if (invTask.type != InvTask.START)	//不是start的话，不予考虑
//					continue;
//				if (sucTasks.contains(postLogName))	//包含了，需要添加
//				{
//					AuditTrailEntryImpl ate = new AuditTrailEntryImpl();
//					String invTaskName = invTask.taskName;
//					int splitPos = invTaskName.indexOf("\0");
//					String taskElement = invTask.taskName;
//					String taskType = "auto";
//					ate.setElement(taskElement);
//					ate.setType(taskType);
//					result.insert(ate, pos);
//				}
//
//			}
//			else if (pos+1 == result.size() && invTask.type == InvTask.END)
//			{
//				if (preTasks.contains(postLogName))	//包含了，需要添加
//				{
//					AuditTrailEntryImpl ate = new AuditTrailEntryImpl();
//					String invTaskName = invTask.taskName;
//					int splitPos = invTaskName.indexOf("\0");
...
//					ate.setElement(taskElement);
//					ate.setType(taskType);
//					result.insert(ate, pos+1);
//				}
//			}
//			else
//			{
//				if (invTask.type != InvTask.SKIP_REDO_SWITCH)
//					continue;
//				AuditTrailEntry preAte = result.get(pos-1);
//				preLogName = preAte.getElement() + "\0" + postAte.getType();
//				if (preTasks.contains(preLogName) && sucTasks.contains(postLogName))	//包含了，需要添加
//				{
//					AuditTrailEntryImpl ate = new AuditTrailEntryImpl();
//					String invTaskName = invTask.taskName;
//					int splitPos = invTaskName.indexOf("\0");
...
//        return "<h1>" + getName() + "</h1><p>For more information, please refer to publication:<br>&nbsp;&nbsp;&nbsp;Lijie Wen, Jianmin Wang and Jiaguang Sun<br>&nbsp;&nbsp;&nbsp;<i>Detecting Implicit Dependencies between Tasks from Event Logs</i><br>&nbsp;&nbsp;&nbsp;In Xiaofang Zhou et al.(Eds.): APWeb 2006, LNCS 3841, pp.591-603, 2006. Springer-Verlag Berlin Heidelberg 2006</p>";
//    }
//
...
//                    HashSet<String> preRelation = new HashSet<String>();
//                    HashSet<String> succRelation = new HashSet<String>();
//
//                    preRelation.add(pre);
//                    preRelation.add(s);
//
...
//    HashSet<HashSet<String>> adjustSharpRelation;
//
//    //constructor
//    public RelationMatrix() {
//        htRelations = new Hashtable();
//        adjustSharpRelation = new HashSet<HashSet<String>>();
//    }
//
