...
//        combineInvisibleTaskSup(petrinet);
    ...
    */
    ...
    private boolean isCombinable(HashSet<Place> sarIPredH, HashSet<Place> sarSuccH,
        ...
        for (int j = 0; j < sarIPred.length; j++) {
            ...
            for (int k = 0; k < sarIPredPred.length; k++) {
                ...
                for (int n = 0; n < sarSucc.length; n++) {
                    HashSet<Transition> sarSuccSuccH = sarSucc[n].getSuccessors();
                    ...
                    for (int m = 0; m < sarSuccSucc.length; m++) {
                        if (sarSuccSucc[m].getLogEvent() == null)
                            continue;
                        String preName = sarIPredPred[k].getIdentifier() + '\0' + sarIPredPred[k].getLogEvent().getEventType();
                        String succName = sarSuccSucc[m].getIdentifier() + '\0' + sarSuccSucc[m].getLogEvent().getEventType();
	...
	private void combineInvisibleTask(PetriNet petriNet)
    {
		...
		for (Transition transition	:	petriNet.getTransitions())
		{
			if (transition.getLogEvent() != null)
				continue;
			HashSet<ModelGraphVertex> prePlaces = transition.getPredecessors();
